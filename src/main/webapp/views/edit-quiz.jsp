<%--
  File: edit-quiz.jsp
  Description: Page for teachers to edit an existing quiz (title, description, questions)
               with an improved interactive interface.
--%>
...
<head>
    ...
    <title>Edit Quiz - <c:out value="${quizCoursework.title}"/></title>
    ...
    <style>
        ...
        .question-card { border: 1px solid #ccc; padding: 15px; margin-bottom: 10px; border-radius: 5px; background-color: #fafafa; }
        .question-header { display: flex; justify-content: space-between; align-items: center; }
        .btn-remove-question { color: red; background: none; border: none; font-size: 1.25rem; }
        .option-group { display: flex; align-items: center; margin-bottom: 5px; }
...
<div class="container">
    ...
    <h2>Edit Quiz: <c:out value="${quizCoursework.title}"/></h2>
    <p><strong>Course:</strong> <c:out value="${quizCoursework.course.title}"/></p>

    <form method="POST" action="<c:url value='/teacher/coursework/${quizCoursework.id}/quiz/update'/>" class="form-styled" id="quizForm">
        <%-- Include CSRF token if you’re using Spring Security --%>
        <div class="mb-3">
            <label for="title" class="form-label">Quiz Title:</label>
            <input type="text" name="title" id="title" class="form-control" value="<c:out value='${quizCoursework.title}'/>" required>
        </div>

        <div class="mb-3">
            <label for="description" class="form-label">Description:</label>
            <textarea name="description" id="description" class="form-control" rows="3"><c:out value="${quizCoursework.content}"/></textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <button type="button" class="btn btn-success mt-2" id="addQuestionBtn">+ Add Question</button>
            <small class="form-text text-muted">
                Use the interactive builder to update the quiz questions. Each question requires:
        ...
        <a href="<c:url value='/teacher/course/${quizCoursework.course.id}/manage'/>" class="btn btn-secondary">Cancel</a>
    </form>
</div>
