<%-- File: manage-course.jsp Description: Page for teachers to manage a specific course, including adding/viewing
    coursework. This version is self-contained. --%>
                ...
                <%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
                    ...
                    <head>
                        ...
                        <style>
                            ...
                            .assignment-submissions-details td {
                                background-color: #f9f9f9 !important;
                                padding: 15px;
                            }

                            /* !important to override tr:hover */
                            .footer {
                                text-align: center;
                                padding: 20px 0;
                                margin-top: 30px;
                    ...
                    <body>
                        ...
                        <div class="container">
                            ...
                            <p><strong>Teacher:</strong>
                                <c:out value="${course.teacher.fullName}" />
                            </p>

                            <hr>

                            <h3>Add New Coursework</h3>
                            <form method="POST" action="<c:url value='/teacher/course/${course.id}/coursework/add'/>"
                                enctype="multipart/form-data" class="form-styled">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" id="courseworkType" class="form-control"
                                        onchange="toggleCourseworkFields()" required>
                                        <option value="">-- Select Type --</option>
                                        <c:forEach items="${courseworkTypes}" var="cwType">
                                            <option value="${cwType}">${fn:toLowerCase(fn:replace(cwType, '_', ' '))}
                                            </option>
                                        </c:forEach>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    <label for="courseworkContent" id="courseworkContentLabel">Description /
                                        Content:</label>
                                    <textarea name="content" id="courseworkContent" class="form-control"
                                        rows="4"></textarea>
                                </div>
                                ...
                                <div id="quizFields" class="form-group" style="display:none;">
                                    ...
                                    <textarea name="quizQuestionsText" id="quizQuestionsText" class="form-control"
                                        rows="10"
                                        placeholder="Enter questions one by one, separated by '---'. Mark correct option with [*] e.g., B) Option B [*]"></textarea>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        <c:forEach items="${courseworkList}" var="cw">
                                            <tr>
                                                <td>
                                                    <c:out value="${cw.title}" />
                                                </td>
                                                <td>
                                                    <c:out value="${fn:toLowerCase(fn:replace(cw.type, '_', ' '))}" />
                                                </td>
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${cw.type == 'NOTE' && not empty cw.filePath}">
                                                            <c:out value="${fn:substring(cw.content, 0, 100)}" />
                                                            <c:if test="${fn:length(cw.content) > 100}">...</c:if><br>
                                                            File: <a
                                                                href="<c:url value='/teacher/coursework/note/${cw.id}/viewfile'/>"
                                                                target="_blank">
                                                                <c:out value="${cw.originalFileName}" />
                                                            </a>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <c:out value="${fn:substring(cw.content, 0, 150)}" />
                                                            <c:if test="${fn:length(cw.content) > 150}">...</c:if>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <c:if test="${cw.type == 'ASSIGNMENT'}">
                                                        <button
                                                            onclick="toggleSubmissions('submissions_cw_${cw.id}'); return false;"
                                                            class="btn btn-info btn-sm">View Submissions</button>
                                                    </c:if>
                                                    <c:if test="${cw.type == 'QUIZ'}">
                                                        <a href="<c:url value='/teacher/quiz/${cw.id}/submissions'/>"
                                                            class="btn btn-info btn-sm">View Submissions</a>
                                                        <a href="<c:url value='/teacher/coursework/${cw.id}/quiz/edit'/>"
                                                            class="btn btn-secondary btn-sm">Edit Quiz</a>
                                                    </c:if>
                                                    <form method="POST"
                                                        action="<c:url value='/teacher/coursework/${cw.id}/delete'/>"
                                                        style="display:inline-block;"
                                                        onsubmit="return confirm('Are you sure you want to delete this coursework? This action cannot be undone and will delete associated submissions/files.');">
                                            ...
                                            <c:if test="${cw.type == 'ASSIGNMENT'}">
                                                <tr id="submissions_cw_${cw.id}" class="assignment-submissions-details"
                                                    style="display:none;">
                                                    <td colspan="4">
                                                        <h4>Submissions for:
                                                            <c:out value="${cw.title}" />
                                                        </h4>
                                                        <%-- Construct the key for accessing submissions from
                                                            requestScope --%>
                                                            <c:set var="assignmentSubmissionsKey"
                                                                value="submissions_for_cw_${cw.id}" />
                                                            <c:set var="assignmentSubmissions"
                                                                value="${requestScope[assignmentSubmissionsKey]}" />
                                                            ...
                                                            <c:if test="${not empty assignmentSubmissions}">
                                                                <ul class="list-group">
                                                                    <c:forEach items="${assignmentSubmissions}"
                                                                        var="sub">
                                                                        <li class="list-group-item">
                                                                            <c:out value="${sub.student.fullName}" /> (
                                                                            <c:out value="${sub.student.email}" />) -
                                                                            <a
                                                                                href="<c:url value='/teacher/assignment/submission/${sub.id}/download'/>">Download
                                                                                Submission</a>
                                                                            (Submitted:
                                                                            <fmt:formatDate
                                                                                value="${sub.submissionDateAsUtilDate}"
                            ...
                            <br>
                            <a href="<c:url value='/teacher/dashboard'/>" class="btn btn-secondary">Back to
                                Dashboard</a>
                        </div>
